...
const commands = ['check', 'plan', 'start', 'stop', 'status', 'doctor', 'acceptance'];
if (!command || !manifestArgument || !commands.includes(command)) {
  console.error('Usage: node dist/identity-cli.js <check|plan|start|stop|status|doctor|acceptance> <identity-manifest.json>');
  process.exit(2);
}
...
try {
  ...
  else if (command === 'start') result = runtime.start(manifest);
