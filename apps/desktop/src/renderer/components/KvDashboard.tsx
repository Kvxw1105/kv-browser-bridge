...
export function KvDashboard({ openConsole, openFolder, newProject }: { openConsole(): void; openFolder(): void; newProject(): void }) {
  ...
  const [discovery, setDiscovery] = useState<{ chrome?: { recommended?: string; candidates?: Array<{ path: string }> }; proxies?: Array<{ protocol: string; host: string; port: number; processName?: string }>; identityRoot?: string; scannedAt?: string }>();
  ...
  const scan = async (): Promise<void> => { setLoading(true); try { const result = await window.identityConsole.discover(); if (result.ok) setDiscovery(result.data as typeof discovery); } finally { setLoading(false); } };
