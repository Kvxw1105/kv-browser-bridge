...
export type ComputerAction = {
  ...
  type: 'launch_app' | 'focus_window' | 'click_ref' | 'click_point' | 'type_text' | 'hotkey' | 'wait';
  [key: string]: unknown;
};
...
export interface ActionReceipt {
  ...
  verification: { status: VerificationStatus; evidence?: Record<string, unknown> };
}

const readCommands = new Set([
  'browser_get_tabs', 'browser_find', 'browser_download_status', 'browser_list_bookmarks',
  'browser_list_extensions', 'browser_snapshot', 'browser_screenshot', 'browser_wait_for',
  'browser_get_text', 'browser_get_url', 'browser_console_logs', 'browser_console_errors',
  'browser_network_requests', 'browser_network_failures', 'browser_get_response_body',
  'browser_inspect_element', 'browser_get_element_styles', 'browser_page_metrics',
  'browser_connection_status',
]);

export function classifyActionRisk(action: ComputerAction): RiskLevel {
  if (action.type !== 'browser_command') return action.type === 'wait' ? 'read' : 'reversible-write';
...
export function validateActionEnvelope(envelope: ActionEnvelope): string[] {
  ...
  if (envelope.risk !== expectedRisk && envelope.risk !== 'destructive') {
    errors.push(`risk mismatch: expected ${expectedRisk}`);
  }
  if ((envelope.risk === 'external-write' || envelope.risk === 'destructive') && envelope.approved !== true) {
